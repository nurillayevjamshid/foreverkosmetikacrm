...
# Snippets to find
snippet_before = '<!-- Product Details Modal -->'
snippet_after = '<button type="button" class="select-option selected" data-value="manager">'

# We'll replace everything from after productDetailsModal end up to snippet_after
# Find the end of productDetailsModal
prod_modal_end_marker = '</div>\n        </div>\n    </div>'
start_pos = content.find(prod_modal_end_marker, content.find(snippet_before))
if start_pos == -1:
    prod_modal_end_marker = '</div>\r\n        </div>\r\n    </div>'
    start_pos = content.find(prod_modal_end_marker, content.find(snippet_before))

if start_pos != -1:
    start_pos += len(prod_modal_end_marker)
    
    end_pos = content.find(snippet_after)
                ...
                <div class="form-group">
                    ...
                    <div class="select-picker" id="newUserRolePicker">
                        ...
                        <div class="select-dropdown">
                            <button type="button" class="select-option" data-value="admin">
                                <i class="fas fa-user-shield"></i>
                                <span>Admin (To'liq huquq)</span>
                            </button>
"""
        new_content = content[:start_pos] + middle_part + content[end_pos:]
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(new_content)
        print("Success")
    else:
        print("End snippet not found")
else:
